...
def index():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # 업로드된 파일이 로컬에서 쓰던 엑셀과 동일하게 읽히는지 검증 (필수 컬럼 확인)
            try:
                ...
                if missing:
                    flash(f"엑셀에 필수 컬럼이 없습니다: {', '.join(missing)}. 시프티 출퇴근 형식인지 확인해 주세요. (현재 컬럼: {list(trial.columns)[:10]}…)", "error")
                    return render_template("upload.html")
            ...
            try:
                ...
                run_pipeline(input_path=input_path, output_dir=output_dir, leave_path=leave_path)
            except Exception as e:
            ...
            if daily.empty or payroll.empty:
                flash("처리 후 근무 데이터가 없습니다. 엑셀에 사원번호·직원·날짜·출근시간·퇴근시간 컬럼이 있는지 확인해 주세요.", "error")
                return render_template("upload.html")

            # 날짜 컬럼 순서(payroll과 동일): daily 기준 정렬
            ...
            date_cols_in_payroll = [c for c in payroll.columns if c not in ["employee_id", "employee_name", "first_attendance_date"] + pay_cols]
            ...
            # 셀 정보: (컬럼명, date_key 또는 None, "date"|"weekly") — 주휴는 해당 주 오른쪽에 배치된 순서 유지
            payroll_cell_list = []
            ...
            # 비소정요일(예: MTW 계약의 목·금) + 출퇴근 없음 → 빈칸 흰색(결근 아님)
            ...
            # 계약별 소정근로시간(시간) — 빈칸 자동채우기에서 계약별로 적용용
            # 1) contract_config 우선(직원별 계약 보장), 2) daily의 scheduled_minutes, 3) 기본 8h
            emp_sched_min = {}
            ...
            if CONTRACT_CONFIG_PATH.exists():
                try:
                    ...
                    pass
            _ref_date = "2025-01-06"  # 월요일 — 대부분 계약의 근무일
            ...
            # 사번 없음(미지정_N) 행용: 뷰에서 계약 선택 드롭다운 옵션
            contract_options = []
            if contract_types:
            ...
            # emp_type: 전체보기/정규직/상용직/프리랜스 필터용
            ...
            for row in payroll_rows:
                ...
                # 8시간 초과 컬럼(옅은 보라색용) — 표시값(계약 cap 적용) 기준으로만
                over_8_cols = set()
                sched_hrs = row.get("_scheduled_hours")
                sched_f = float(sched_hrs) if sched_hrs is not None and str(sched_hrs).strip() else 8.0
                for c in date_cols_in_payroll:
                    v = row.get(c)
                    if v is None or v == "":
                        continue
                    try:
                        ...
                        if not s:
                            continue
                        raw = float(s)
                        # 셀에 노출되는 값(15.0, 16.0 등) 기준으로 8시간 초과 보라색 적용
                        if raw > 8.0:
                            over_8_cols.add(c)
                    except (TypeError, ValueError):
                        pass
            ...
            # 빈칸 자동채우기 후 클라이언트 재계산 시 프리랜서 명절(공휴일) 근무 = 전부 야근 반영용
            holiday_cols = {}
            freelancer_ids = {}
            ...
            # 급여 컬럼: 금액은 천 단위 콤마, 시간 컬럼은 소수 1자리
            ...
            # 결과 HTML 생성은 with 블록 안에서 (데이터 확보 후 바로 응답)
            try:
...
def export_google_sheet():
    """payroll_result를 구글 시트로 내보내기. session의 last_run_id 사용."""
    if request.method == "GET":
        return jsonify({"ok": True, "message": "POST로 요청하세요."})

    run_id = session.get("last_run_id")
    if not run_id:
        return jsonify({"ok": False, "error": "내보낼 결과가 없습니다. 먼저 엑셀을 업로드해 결과를 생성해 주세요."}), 400

    output_dir = OUTPUT_BASE / run_id
    if not output_dir.exists():
        return jsonify({"ok": False, "error": "결과 파일이 만료되었습니다. 다시 업로드 후 시도해 주세요."}), 404
