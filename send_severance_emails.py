"""
이메일_발송용_정보 시트 기반 자동 퇴직금 산정 발송기.

동작:
1) 터미널에서 YYYYMM(예: 202603)을 입력받아 해당 월의
   '{YYYY}년{MM:02d}월_이메일_발송용_정보' 시트를 찾습니다.
2) 터미널에서 로컬 PDF 폴더 경로를 입력받고,
   시트의 '이메일주소'와 '첨부파일' 값을 기준으로 PDF 파일명을 매칭해 첨부 발송합니다.
   (제목/본문은 변수 없이 고정 문구입니다)
...
SPREADSHEET_ID = "1PKHSowsKB2QwFr2rpbZNdkIGWQJ8QtmNCpjHBCpPorM"

# 시트에서 읽을 열 위치(0-based). 헤더 이름이 아니라 위치로 읽습니다.
COL_EMAIL_IDX = 5   # F열: 이메일주소
COL_ATTACH_IDX = 6  # G열: 첨부파일(로컬 PDF 파일명과 매칭)
...
def _sheet_title_from_yyyymm(year: int, month: int) -> str:
    # 예: 202608 -> '2026년08월_퇴직정산_이메일_발송용'
    return f"{year}년{month:02d}월_퇴직정산_이메일_발송용"
...
def _records_from_worksheet(ws) -> list[dict]:
    """
    헤더 이름이 아니라 '열 위치'로 읽습니다. (F열=이메일주소, G열=첨부파일)
    헤더 문구가 시트마다 달라도 동작하며, 첫 행은 헤더로 보고 건너뜁니다.
    단, 첫 행 F열에 이미 이메일이 들어있으면 헤더가 없는 것으로 보고 데이터에 포함합니다.
    """
    values = ws.get_all_values()  # [ [header...], [row...], ... ]
    if not values:
        return []

    def _at(row: list, idx: int) -> str:
        return str(row[idx]).strip() if len(row) > idx else ""

    rows = values
    if "@" not in _at(values[0], COL_EMAIL_IDX):
        rows = values[1:]  # 첫 행은 헤더

    records: list[dict] = []
    for row in rows:
        records.append(
            {
                "이메일주소": _at(row, COL_EMAIL_IDX),
                "첨부파일": _at(row, COL_ATTACH_IDX),
            }
        )
    return records
...
def _build_email_body(row: dict, year: int, month: int) -> tuple[str, str]:
    # 제목/본문 모두 고정 문구입니다. (시트 값은 본문에 넣지 않습니다)
    subject = "[마인이스] 퇴직금 산정 송부"

    body = "\n".join(
        [
            "안녕하세요, 마인이스의 챈스입니다.",
            "",
            "첨부와 같이 퇴직금 산정을 전달드립니다. 지급은 오늘 완료되었습니다.",
            "",
            "궁금하신 부분 있으면 언제든 연락주세요.",
            "",
            "감사합니다,",
            "챈스 드림",
        ]
...
def main():
    parser = argparse.ArgumentParser(description="이메일_발송용_정보 시트 기반 퇴직금 산정 자동 발송기")
